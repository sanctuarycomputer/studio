...
function main1() {
  ...
  */
  ...
  arrayLookups["avg"] = `${(arrayTotal / BENCHMARK_ROUNDS).toFixed(2)}ns`;
  ...
  */
  ...
  for (let i = 0; i < BENCHMARK_ROUNDS; i++) {
    ...
    const objectBenchmarkDuration =
      (objectBenchmarkEnd - objectBenchmarkStart) / 1000;
    objectLookups[i + 1] = `${objectBenchmarkDuration.toFixed(
      2
    )} (${randomKey})`;
    objectTotal = objectTotal + objectBenchmarkDuration;
  }
  objectLookups["avg"] = `${(objectTotal / BENCHMARK_ROUNDS).toFixed(2)}ns`;
